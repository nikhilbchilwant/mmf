...
@registry.register_model("mutan_bert")
class MutanBERT(BaseModel):
    def __init__(self, config, *args, **kwargs):
        super().__init__(config)
        self._is_direct_features_input = config.direct_features_input
    ...
    @classmethod
    def config_path(cls):
        return "configs/models/fusions/mutan_bert.yaml"
    ...
    def build(self):
        ...
        # classifier_config.params.in_dim = num_features * self.config.modal_hidden_size
        # classifier_config.params.in_dim += self.config.text_hidden_size
        classifier_config.params.in_dim = 16 #Batch size
        self.classifier = build_classifier_layer(classifier_config)

        self.fusion = fusions.Mutan([self.config.text_hidden_size, self.config.modal_hidden_size], classifier_config.params.in_dim, mm_dim=20)
    ...
    def get_optimizer_parameters(self, config):
        ...
        parameters += get_bert_configured_parameters(self.classifier, lr)
        parameters += get_bert_configured_parameters(self.fusion, lr)
        return parameters
    ...
    def forward(self, sample_list):
        ...
        text_embedding, modal_embedding = self.base(text, modal, [mask, segment])
        # print(f'text_embedding.shape = {text_embedding.shape}, modal_embedding.shape = {modal_embedding.shape}')
        # embedding = torch.cat([text_embedding, modal_embedding], dim=-1)
        embedding = self.fusion([text_embedding, modal_embedding])
        output = {}
        output["scores"] = self.classifier(embedding)
        return output
